#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Placement pipeline — container detection/grounding, safe-region sampling,
two-phase approach with re-observation, release and post-placement verification.

Extracted from skills/place.py so the PlaceSkill stays a thin CLI shell and
other skills (pick_and_place, fetch_from_user) can reuse the same placement
implementation on the same skill instance (no extra sockets).
"""
...
class PlacePipeline:
    """Placement chain composed around an existing Skill.
    ...
    def place_at_named_pose(self, pose_name, side="left", approach_speed=15,
                            retreat_speed=30, release_wait=1.0):
        """Release at a configured fixed pose using the common place contract.

        This is the fixed-target counterpart of :meth:`run`; it is used for
        desks, trash and drawer receptacles that do not need visual grounding.
        """
        pose = self.config.get_pose(pose_name, side=side)
        if pose is None:
            cprint("[place] missing fixed placement pose: %s" % pose_name, "red")
            return False
        if not self.control_arm(
                pose_type=pose_name, speed=approach_speed, side=side):
            return False
        if not self.control_hand(cmd_type="open", side=side):
            return False
    ...
    def _vlm_container_detection_attempts(self, class_name, image):
        ...
        prompts are sent back through YOLOE.  Neither result directly
        ...
        # YOLOE again on the same frame: besides being redundant, its
